...
Reduce costs and latency by caching static or repetitive prompt content (like system prompts, tool definitions, and conversation history) on Anthropic's servers. When the same content appears in subsequent requests within the cache TTL, it's retrieved from cache instead of being reprocessed, significantly reducing costs. Prompt caching is useful for the following:
